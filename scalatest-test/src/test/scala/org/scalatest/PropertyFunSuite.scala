...
class PropertyFunSuite extends FunSuite {
  ...
  test("object has no appropriately named field, method, or get method (0, 0, 0)") {
    ...
    val result = MatchersHelper.accessProperty(dgn, sym"fred", false)
  ...
  test("object has only an appropriately named get method (0, 0, 1)") {
    ...
    val result = MatchersHelper.accessProperty(obj, sym"cow", false)
  ...
  test("object has only an appropriately named Boolean is method (0, 0, 1)") {
    ...
    val result = MatchersHelper.accessProperty(obj, sym"cow", true)
  ...
  test("works on set.empty") {
    val result1 = MatchersHelper.accessProperty(Set(), sym"empty", true)
    result1 should be (Some(true))
    val result2 = MatchersHelper.accessProperty(Set(1, 2, 3), sym"empty", true)
